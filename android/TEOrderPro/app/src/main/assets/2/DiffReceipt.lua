...
DiffReceipt.__index=DiffReceipt
...
function DiffReceipt:new( key, language, orderDetail )
	-- body
	local self={}
	setmetatable(self,DiffReceipt)
...
-- print title
function DiffReceipt:printTopTitle(isReprint)
	-- body
	-- 店铺名称
    ...
    local orderType
   	 -- 桌号  订单类型
	if(self.odData.type == 2) then
	    tableNO = self.odData.tableCode
    ...
    self.printer:goPaperPoints(printH2)
       ...
       typeName = self.localInfo:getReceiptType("refund")
       self.printer:setColorReversal(true)
   	else
       if self.isReprint == true then
          typeName = 'receipt_rep'
          self.printer:setColorReversal(true)
       else
           typeName = 'receipt'
       end
...
function DiffReceipt:printDishes(dishList)
...
function DiffReceipt:printDishItem( item )
	-- body
	-- 打印菜品的数量，名称，计价
   ...
   self.printer:setFontBold(true)
   self.printer:justifyThreeText(ltxt, ctxt, rtxt,
                                 {left = 6, right = 8}, 2)
...
function DiffReceipt:printDishItemOptions(optionList, indent)
...
function DiffReceipt:printEndTitle()
   	-- 分割线
   	self.printer:initPrinter()
   	self.printer:printLineSpliter()
   	self.printer:goPaperPoints(1)
   	...
   	self.printer:initPrinter()
   	self.printer:setAlignment(1)
   	-- Payment time NOT current time os.date('%Y-%m-%d %H:%M:%S')
   	self.printer:sendText('Payment made at ' .. self.odData.update_date_time)
   	self.printer:goPaperLines(1)
   	self.printer:sendText('Thank you & Have a nice day!')
    self.printer:goPaperPoints(10)
    self.printer:setFontBold(true)
...
function DiffReceipt:Oven_Receipt()
	-- body
		-- 总价格 折扣信息 实收 找回等
   	self.printer:initPrinter()
    ...
    r = string.format("-%.2f", self.odData.discountAmount)
   	...
   	paymentWay=self.localInfo:payType()[self.odData.payment.paymentMethod]
   	l = self.localInfo:translate(paymentWay)
   	if TE_Tag=="cancel_order" or TE_Tag == "cancel_order_pay" then
      	r = string.format("%.2f","-"..self.odData.payment.received)
   	else
      	r = string.format("%.2f", self.odData.payment.received)
   	end
   	...
   	l = self.localInfo:translate("Change:")
   	r = string.format("%.2f", self.odData.payment.change)
   	self.printer:justifyText(l, r)
   	self.printer:goPaperPoints(60)
end


function DiffReceipt:JDC_Receipt( )
	-- body
	 -- 总价格 折扣信息 实收 找回等
   self.printer:initPrinter()
   local l = self.localInfo:translate("Sub Total:")
   ...
   l = self.localInfo:translate("7% GST(Inc):")
   r = string.format("%.2f",self.odData.payPrice - (self.odData.payPrice / 1.07))
   self.printer:justifyText(l, r)
   self.printer:goPaperPoints(60)
   ...
   l = self.localInfo:translate("Cash Tendered:")
   r = string.format("%.2f", self.odData.payment.received)
   self.printer:justifyText(l, r)
   self.printer:goPaperPoints(60)
...
-- CYJ
function DiffReceipt:CYJ_Receipt()
  -- body
    -- 总价格 折扣信息 实收 找回等
    self.printer:initPrinter()
    self.printer:setFontBold(true)
