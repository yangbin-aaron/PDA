...
import com.toughegg.andytools.http.method.SKYunHttpException;
import com.toughegg.andytools.systemUtil.AndyLoger;
import com.toughegg.andytools.systemUtil.HttpUtils;
...
public class FileRequest extends Request<byte[]> {
    ...
    public FileRequest(String storeFilePath, String url, HttpCallBack callback) {
        this(HttpMethod.GET, storeFilePath, url, callback);
    }

    public FileRequest(int method, String storeFilePath, String url, HttpCallBack callback) {
    ...
    public Response<byte[]> parseNetworkResponse(NetworkResponse response) {
        ...
        return Response.error(new SKYunHttpException(errorMessage));
    }
    ...
    public byte[] handleResponse(HttpResponse response) throws IOException, SKYunHttpException {
        HttpEntity entity = response.getEntity();
        long fileSize = entity.getContentLength();
        if (fileSize <= 0) {
            AndyLoger.debug("Response doesn't present Content-Length!");
        }
        ...
        if (fileSize > 0 && mStoreFile.length() == fileSize) {
            ...
            mRequestQueue.getHttpConfig().mIDelivery.postDownloadProgress(this, fileSize, fileSize);
            return null;
        }
        ...
        try {
            ...
            while ((offset = in.read(buffer)) != -1) {
                ...
                mRequestQueue.getHttpConfig().mIDelivery.postDownloadProgress(this, fileSize, downloadedSize);
