...
public class DownloadTaskQueue {
    ...
    private AndyHttp mRequestQueue; // 关联一个请求队列，目的是在恢复下载的时候可以再次将下载请求加入到请求队列中

    public DownloadTaskQueue(int parallelTaskCount) {
        if (parallelTaskCount >= AndyConstants.NETWORK_POOL_SIZE) {
            parallelTaskCount = AndyConstants.NETWORK_POOL_SIZE - 1;
        }
    ...
    public void setRequestQueue(AndyHttp requestQueue) {
        this.mRequestQueue = requestQueue;
    }

    AndyHttp getRequestQueue() {
