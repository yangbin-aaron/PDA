...
public class DiskCache implements ICache {
    ...
    public synchronized void clear() {
        ...
        mEntries.clear();
        mTotalSize = 0;
        AndyLoger.debug("disk Cache cleared.");
    }
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            AndyLoger.debug("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                AndyLoger.debug("Unable to create cache dir %s", mRootDirectory.getAbsolutePath());
            }
            return;
        }
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (!success) {
                fos.close();
                AndyLoger.debug("Failed to write header for %s", file.getAbsolutePath());
        ...
        if (!deleted) {
            AndyLoger.debug("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            AndyLoger.debug("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (HttpConfig.DEBUG) {
            AndyLoger.debug("Pruning old cache entries.");
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (deleted) {
                ...
                AndyLoger.debug("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
            }
            iterator.remove();
            prunedFiles++;
        ...
        if (HttpConfig.DEBUG) {
            AndyLoger.debug("pruned %d files, %d bytes, %d ms", prunedFiles, (mTotalSize - before), SystemClock.elapsedRealtime() - startTime);
        }
    }
